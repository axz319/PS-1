...
# Вертикальный взлёт до 1 км
while altitude_stream() <= 1_000:
    time.sleep(0.2)
...
while True:
    ...
    if fuel == 0:
        ...
        # Обновляем ресурсы второй ступени
        decouple_stage_resources = vessel.resources_in_decouple_stage(
...
# Продолжаем разворот после отделения
time.sleep(1)
control.pitch = -0.96
control.sas_mode = vessel.control.sas_mode.prograde
...
while True:
    ...
    if pitch <= 5 and not pitch_fixed:
        control.pitch = 0
        control.sas_mode = vessel.control.sas_mode.stability_assist
        pitch_fixed = True
    
    # Корректировка отрицательного наклона
    if pitch < 0:
        control.pitch = 1
        time.sleep(1)
        control.pitch = 0
...
while periapsis_stream() < TARGET_PE:
    time.sleep(0.02)
